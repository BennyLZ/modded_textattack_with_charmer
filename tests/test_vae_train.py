...
try:
    ...
    # legacy fallback (older wandb only)
    tb_patch = None
...
def kl_softmax(p_logits, q_logits, T=2.0):
    ...
    # KL(softmax(p) || softmax(q)) over classes, then mean over batch
...
def ta_adv_batch(attack: Attack, texts, labels):
    ...
    args = AttackArgs(
        ...
        shuffle=False,
        disable_stdout=True,   # quiet
    )
    attacker = Attacker(attack, data, args)
    adv_texts = []
    for (x, y), res in zip(data, attacker.attack_dataset()):
        s = None
        if getattr(res, "perturbed_result", None):
            s = res.perturbed_result.attacked_text.text
...
def train_with_vae_consistency(model, tokenizer, attack, train_ds, eval_ds, device):
    ...
    use_vae = True   # flip off until your VAE is ready
    vae = load_your_vae().to(device).eval() if use_vae else None
